...
export default function AvoirForm({ store }) {
  const { id } = useParams();
  const navigate = useNavigate();
  const { state, addAvoir, updateAvoir } = store;
  const isNew = id === 'nouveau' || !id;

  const existing = (state.avoirs || []).find((a) => a.id === id);
  ...
  return (
    <div className={styles.page}>
      <div className="page-header">
        <Link to="/avoirs" className="btn btn-ghost">
          <ArrowLeft size={18} /> Retour
        </Link>
        <h1 className="page-title">
          {isNew ? 'Nouvel avoir' : `Avoir ${existing?.ref ?? ''}`}
        </h1>
        <div className={styles.headerActions}>
          ...
          <button type="submit" form="avoir-form" className="btn btn-primary">
            <Save size={18} /> Enregistrer
          </button>
        </div>
      </div>

      <form id="avoir-form" onSubmit={handleSave} className={styles.form}>
        ...
        <div className="card" style={{ marginTop: '1rem' }}>
          ...
          <textarea
            ...
            placeholder="Motif de l'avoir, référence facture..."
          />
        </div>
      </form>
