...
export default function DevisForm({ store }) {
  const { id } = useParams();
  const navigate = useNavigate();
  const { state, addDevis, updateDevis } = store;
  ...
  return (
    <div className={styles.page}>
      <div className="page-header">
        ...
        <h1 className="page-title">{isNew ? 'Nouveau devis' : `Devis ${existing?.ref ?? ''}`}</h1>
        <div className={styles.headerActions}>
          ...
          <button type="submit" form="devis-form" className="btn btn-primary">
            <Save size={18} /> Enregistrer
          </button>
        </div>
      </div>

      <form id="devis-form" onSubmit={handleSave} className={styles.form}>
